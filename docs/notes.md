...
2. Methods: for `wsn` wireless network, run strategies `hybrid_opt`, `bandwidth_first`, `energy_first`; for `jitter` wireless network, run strategies `hybrid_opt`, `sync`, `async`, `bridge_free`, `bandwidth_first`, `energy_first`.
3. Factors:
- Wireless: `simulated_mode = jitter` and `wsn`.
- Non-IID: `alpha = 0.5` only.
- Seed: 1 seed (e.g., 42).
- Algorithm: `fedprox` with `mu=0.1`
4. Bridge invariants:
...
7. Outputs: per-metric line plots (all strategies, use `generate_comparison_plots.py`); summary tables; Pareto scatter (total upload time vs final accuracy).
