...
TODO:
1) SCAFFOLD 训练与控制变元未实现；2) 无线建模为简化近似（未包含 3GPP/TR 38.901 等复杂模型）；3) 未集成 MOGA‑FL（NSGA‑III/MOEA/D）优化与自动配置回写；4) 数据价值以样本数近似，未引入更丰富的价值估计；5) 桥接态未包含“不变量检查”流程；6) 若仓库模型不可用时会回退到简化 CNN 而非 ResNet‑18 变体；7) 未接入 YAML/实验配置系统（仍为代码内配置）
