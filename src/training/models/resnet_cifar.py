...
class ResNetCIFAR(nn.Module):
    """ResNet-18 变体，适配 CIFAR-10（3x32x32 输入）。
    ...
    def __init__(self, num_classes: int = 10, width_factor: float = 1.0) -> None:
        super().__init__()
        ...
        # 4 个残差 stage，每个 stage 包含 2 个 BasicBlock
...
def build_resnet18_cifar(num_classes: int = 10, width_factor: float = 1.0) -> ResNetCIFAR:
    ...
    width_factor: 通道宽度缩放因子，<1.0 时得到“窄版 ResNet”，适合 quick 模式。"""
    return ResNetCIFAR(num_classes=num_classes, width_factor=width_factor)
