...
class SimpleClient:
    ...
    def local_train(self, global_state, local_epochs=1, batch_size=32, lr=0.01):
        ...
        # MNIST标签平滑略降（0.05），避免过度正则化
        criterion = nn.CrossEntropyLoss(label_smoothing=0.05)

        # 初始化调度器（MNIST训练轮数减少为60）
...
def main():
    # MNIST专属参数
    num_clients = 1
    num_rounds = 60  # MNIST收敛更快，训练60轮足够
    local_epochs = 1
    batch_size = 128
    lr = 0.001
